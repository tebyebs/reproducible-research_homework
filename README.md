...
- A) Each plot represents a co-ordinate matrix with a line following the random trajectory generated by the random_walk function over the time period 0 to 500. For each time step, the path moves a fixed distance in a random direction, from 0 to 360 degrees around the original point. Each time the script is run, the original start location changes, so too does the nature of the path taken and therefore the end location, though the total distance travelled remains the same. Since the script is random, both plot a and b will be different each time.
...
- D) <p align="center">
        <img src="https://github.com/tebyebs/reproducible-research_homework/blob/main/Commit Changes.png" width="407" height="457">
     </p>
